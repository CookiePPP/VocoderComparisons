...
def dynamic_range_compression(x, C=1, clip_val=1e-5, b_ln=True):
    if b_ln:
...
def logmelfilterbank(audio,
    ...
    mel_spc = np.dot(spc, mel_basis.T)
    mel_spc = dynamic_range_compression(mel_spc, clip_val=eps, b_ln=ln_scale_mags)
    return mel_spc
...
def main():
    ...
    for utt_id, (audio, fs) in tqdm(dataset):
        ...
        mel = logmelfilterbank(x,
                               ...
                               fmin=config["fmin"],
                               fmax=config["fmax"]
                               ln_scale_mags=config.get('ln_scale_mags', False))
